...
class MyAdagrad(Optimizer):
    ...
    Arguments:
        ...
        weight_decay (float, optional): weight decay (weght_decay penalty) (default: 0)
    """
...
def get_optimizer(name, parameters, lr, weght_decay=0):
    if name == 'sgd':
        return torch.optim.SGD(parameters, lr=lr, weight_decay=weght_decay)
    elif name in ['adagrad', 'myadagrad']:
        return MyAdagrad(parameters, lr=lr, init_accu_value=0.1, weight_decay=weght_decay)
    elif name == 'adam':
        return torch.optim.Adam(parameters, weight_decay=weght_decay, lr=lr, betas=(0.9, 0.98)) # use default lr
    elif name == 'adamax':
        return torch.optim.Adamax(parameters, weight_decay=weght_decay) # use default lr
    elif name == 'adadelta':
        return torch.optim.Adadelta(parameters, lr=lr, weight_decay=weght_decay)
    else:
